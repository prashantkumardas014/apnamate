...
function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/users`,
        {
          method: "GET",
          headers: {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/bookings`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      setBookingMessage(error.message || "Unable to load bookings");
      setBookings([]);
    } finally {
  ...
  const handleDeleteBooking = async (bookingId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/bookings/${bookingId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      setBookingMessage(`Booking #${bookingId} deleted successfully.`);
      await fetchBookings(user.id);

    } catch (error) {
      console.error("Delete booking error:", error);
      setBookingMessage(error.message || "Unable to delete booking");
    }
  };
  ...
  const handleBlockUser = async (userId, userName) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/users/${userId}/block`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setMessage(`${userName} has been blocked successfully.`);
      await fetchUsers(user.id);

    } catch (error) {
      console.error("Block user error:", error);
      setMessage(error.message || "Unable to block user");
    }
  };
  ...
  const handleUnblockUser = async (userId, userName) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/users/${userId}/unblock`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setMessage(`${userName} has been unblocked successfully.`);
      await fetchUsers(user.id);

    } catch (error) {
      console.error("Unblock user error:", error);
      setMessage(error.message || "Unable to unblock user");
    }
  };
  ...
  const handleDeleteUser = async (userId, userName) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/users/${userId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      setMessage(`${userName} has been deleted successfully.`);
      await fetchUsers(user.id);
  ...
  const handleAcceptBooking = async (bookingId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/bookings/${bookingId}/accept`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setBookingMessage(`Booking #${bookingId} accepted successfully.`);
      await fetchBookings(user.id);

    } catch (error) {
      console.error("Accept booking error:", error);
      setBookingMessage(error.message || "Unable to accept booking");
    }
  };
  ...
  const handleRejectBooking = async (bookingId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/bookings/${bookingId}/reject`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(data?.detail || "Failed to reject booking");
      }

      setBookingMessage(`Booking #${bookingId} rejected successfully.`);
      await fetchBookings(user.id);

    } catch (error) {
      console.error("Reject booking error:", error);
      setBookingMessage(error.message || "Unable to reject booking");
    }
  };
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/bookings/admin/bookings/${bookingId}/cancel`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setBookingMessage(`Booking #${bookingId} cancelled successfully.`);
      await fetchBookings(user.id);

    } catch (error) {
      console.error("Cancel booking error:", error);
      setBookingMessage(error.message || "Unable to cancel booking");
    }
  };
  ...
  const totalCustomers = users.filter((item) => item.role === "customer").length;
  const totalProviders = users.filter((item) => item.role === "provider").length;
  ...
  const acceptedBookings = bookings.filter((item) => item.status === "Accepted").length;
      ...
      {showModal && selectedBooking && (
        ...
        >
          ...
          >
            ...
            >
              <h2 style={{ margin: 0, color: "#2563eb" }}>Booking Details</h2>
              <button
                onClick={() => {
                  setShowModal(false);
                  setSelectedBooking(null);
...
const statusMessageStyle = (backgroundColor, color) => ({
  marginTop: "15px",
  padding: "10px",
  backgroundColor,
  color,
